...
% 
% 
% colors = [ 0, 1, 1; 0, 0.8, 0.8; 0, 0.6, 0.6; 1, 0, 0 ];
% 
% 
% M = mean( M, 3 );
% C = all( C, 3 );
...
% hold on
% boundary_f = zeros( [ size( likelihood_f ), 4 ] );
% boundary_f( 2 : end, 1 : end, 1 ) = ~isnan( likelihood_f( 2 : end, 1 : end ) ) & isnan( likelihood_f( 1 : end - 1, 1 : end ) );
% boundary_f( 1 : end - 1, 1 : end, 2 ) = ~isnan( likelihood_f( 1 : end - 1, 1 : end ) ) & isnan( likelihood_f( 2 : end, 1 : end ) );
% boundary_f( 1 : end, 2 : end, 3 ) = ~isnan( likelihood_f( 1 : end, 2 : end ) ) & isnan( likelihood_f( 1 : end, 1 : end - 1 ) );
% boundary_f( 1 : end, 1 : end - 1, 4 ) = ~isnan( likelihood_f( 1 : end, 1 : end - 1 ) ) & isnan( likelihood_f( 1 : end, 2 : end ) );
% boundary_f = sum( boundary_f, 3 ) > 0;
% pos_x = [];
% pos_y = [];
% for c1 = 1 : size( boundary_f, 1 )
%     for c2 = 1 : size( boundary_f, 2 )
%         if boundary_f( c1, c2 ) == 1
%             pos_x = [ pos_x, pool_period_refractory( c2 ) ];
%             pos_y = [ pos_y, pool_period_initiation( c1 ) ];
%             % plot( pool_period_refractory( c2 ), pool_period_initiation( c1 ), '.k' )
%         end
%     end; clear c2
% end; clear c1
...
% hold on
% plot( [ 0.5, 1500.5 ], [ 500.5, 500.5 ], '-m', 'linewidth', 2 )
% plot( [ 500.5, 500.5 ], [ 0.5, 1500.5 ], '-m', 'linewidth', 2 )
% colorbar% (pF)
% set( gca, 'xtick', [], 'ytick', [] )
% 
% % ct_f = 0;
...
% 
% figure( 'position', [ 100, 100, 300, 200 ] )
% imagesc( 2 * ( 0.5 + 2 ) * pool_I, spkT1, P, [ 1, 4 ] )
% colormap( gca, colors )
% colorbar( 'ticks', [ 0, 1 ], 'ticklabels', { '', '' } )
% axis xy
% hold on
...
% 
% % t_A = 120 - spkT1;
% t_A = 120 * ones( length( spkT1 ), 1 );
% 
% N = 2;
% 
% likelihood_f = ( ( 1 - ( spkT1 ./ ( t_A ) ) ) .^ ( N - 1 ) ) .* ( ( N - 1 ) * spkT1 ./ ( t_A ) );
% likelihood_f = repmat( likelihood_f, [ 1, length( pool_I ) ] );
% 
% likelihood_f( repmat( spkT1, [ 1, length( pool_I ) ] ) <= 0 ) = NaN;
% likelihood_f( bsxfun( @gt, spkT1, transpose( t_A ) / ( N - 1 ) ) ) = NaN;
% likelihood_f( bsxfun( @gt, spkT1, 2 * ( 0.5 + 2 ) * pool_I / ( N - 1 ) ) ) = NaN;
% 
% boundary_f = zeros( [ size( likelihood_f ), 4 ] );
% boundary_f( 2 : end, 1 : end, 1 ) = ~isnan( likelihood_f( 2 : end, 1 : end ) ) & isnan( likelihood_f( 1 : end - 1, 1 : end ) );
% boundary_f( 1 : end - 1, 1 : end, 2 ) = ~isnan( likelihood_f( 1 : end - 1, 1 : end ) ) & isnan( likelihood_f( 2 : end, 1 : end ) );
...
%     for c2 = 1 : size( boundary_f, 2 )
%         if boundary_f( c1, c2 ) == 1
%             pos_x = [ pos_x, 2 * ( 0.5 + 2 ) * pool_I( c2 ) ];
%             pos_y = [ pos_y, spkT1( c1 ) ];
%             % plot( pool_period_refractory( c2 ), pool_period_initiation( c1 ), '.k' )
%         end
%     end; clear c2
% end; clear c1
...
% hold on
% plot( [ 0.5, 1500.5 ], [ 500.5, 500.5 ], '-m', 'linewidth', 2 )
% plot( [ 0.5, 1500.5 ], [ 1000.5, 1000.5 ], '-m', 'linewidth', 2 )
% plot( [ 500.5, 500.5 ], [ 0.5, 1500.5 ], '-m', 'linewidth', 2 )
% plot( [ 1000.5, 1000.5 ], [ 0.5, 1500.5 ], '-m', 'linewidth', 2 )
% colorbar% (pF)
% set( gca, 'xtick', [], 'ytick', [] )
% 
% % ct_f = 0;
...
% set( gca, 'xtick', [ 0 : 1 : 4 ], 'xticklabel', { '0', '1/6', '2/6', '3/6', '4/6' } )
% colormap( colors )
% colorbar( 'ticks', [ 0, 1 ], 'ticklabels', { '', '' } )
% 
...
% 
% figure( 'position', [ 100, 100, 300, 150 ] )
% imagesc( noiseLevel, [ 2 : 6 ], transpose( P_pool ) )
% set( gca, 'xtick', [ 0 : 1 : 4 ], 'xticklabel', { '0', '1/6', '2/6', '3/6', '4/6' } )
% colormap( colors )
% colorbar( 'ticks', [ 0, 1 ], 'ticklabels', { '', '' } )
% 
...
%     likelihood_f( bsxfun( @gt, spkT1, 2 * ( 0.5 + 2 ) * pool_I / ( N - 1 ) ) ) = NaN;
% 
%     likelihood_f_all( ct_N, 1 ) = likelihood_f;
% 
% end; clear N ct_N
% 
% colors = [ 0, 1, 1; 0, 0.8, 0.8; 0, 0.6, 0.6; 1, 0, 0 ];
...
% imagesc( noiseLevel, [ 2 : 6 ], transpose( P_pool ) )
% hold on
% plot( noiseLevel, 3.5 * ones( 1, length( noiseLevel ) ), '-k', 'linewidth', 2 )
% set( gca, 'xtick', [ 0 : 1 : 4 ], 'xticklabel', { '0', '1/6', '2/6', '3/6', '4/6' } )
% colormap( colors )
% colorbar( 'ticks', [ 0, 1 ], 'ticklabels', { '', '' } )
% 
...
%         pts = find( s_E( n, 2 * 50 + [ 1 : 2 * 500 ] ) == 1 );
%         if ~isempty( pts )
%             plot( pts, 2 * ct_n + n - 500 + 50, '.m' )
%         end
%     end; clear n pts
%     for n = ct_n + 1000 + [ 1 : 50 ]
%         pts = find( s_E( n, 2 * 50 + [ 1 : 2 * 500 ] ) == 1 );
%         if ~isempty( pts )
%             plot( pts, 2 * ct_n + n - 1000 + 100, '.b' )
%         end
%     end; clear n pts
%     ct_n = ct_n + 50;
% end
% set( gca, 'xlim', [ 0, 1001 ], 'ylim', [ 0, 1001 ] )
% set( gca, 'xtick', linspace( 1, 1000, 6 ), 'xticklabel', { '0', '100', '200', '300', '400', '500' } )
% disp( [ 'noiseLevel = ', num2str( noiseLevel( ct_nn ) ) ] )
% 
% ct_nn = 5;
% s_E = s_E_all{ ct_nn, 1 };
% figure( 'position', [ 100, 100, 300, 500 ] )
